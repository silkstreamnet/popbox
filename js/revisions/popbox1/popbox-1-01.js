(function($){
	...
	function popboxStart(linkobject,settings)
	{
		...
		$(linkobject).click(function(e){
			...
			setStyle = (setWidth != '' || setHeight != '') ? 'style="'+setWidth+' '+setHeight+'"' : '';
			
			closeText = (typeof(settings.closeText) === "string") ? settings.closeText : closeText;
			var wrapfirst = '<div class="quo-cast"></div><div class="quo-popup" '+setStyle+'><a class="quo-close">'+closeText+'</a>';
			var wraplast = '</div>';
			...
			{
				popup.find('#'+settings.contentid).show();
			}
